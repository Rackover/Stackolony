...
[ExecuteInEditMode]
public class ReCalcCubeTexture : MonoBehaviour //CE SCRIPT PERMET DE TILER LA TEXTURE D'UN MESH POUR EVITER DE LA DEFORMER
{
    ...
    public void Calculate()
    {
        ...
        if (GetComponent<Renderer>().sharedMaterial.mainTexture.wrapMode != TextureWrapMode.Repeat) //On verifie que le mode de texture est bien en "Repeat" sinon on le change
        {
    ...
    private Vector2[] SetupUvMap(Vector2[] meshUVs) //On recalcule les UV pour les adapter à la texture
    {
